...
const Input = ({placeholder,label,type,name,handleChange,value}: Props) => {
  ...
  return (
    <div className="flex flex-col mb-4">
        ...
        <div className="relative">
            <input autoComplete={(type == 'password') ? "off" : "on"} data-testid={`input-${name}`} value={value && value as string} onChange={handleChange} name={name} className="bg-white py-2 px-4 rounded-xl border border-zinc-300 w-full" type={inputType} placeholder={placeholder} />
            {
                (type == 'password') &&
